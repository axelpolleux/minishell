...
/*   Updated: 2026/05/25 15:12:46 by ethutin-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
// {
// 	if (execve(cmd->cmd_path, cmd->args, env) == -1) //  cmd->args ne fonctionnerea pas 
// 	{
// 		free_arr(env);
// 		perror("minishell");
// 		if (errno == ENOENT)
...
void	exec(t_data *data)
{
	...
	t_cmd = data->cmd;
	if (is_builtin(data->built_in, t_cmd->command) && !t_cmd->next)
	{
